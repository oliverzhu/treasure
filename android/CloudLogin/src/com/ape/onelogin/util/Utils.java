...
public class Utils {
    ...
    public static String getPlainUserKey(String userKey) {
        LogUtil log = new LogUtil("Utils");
        String plainUserKey = AbsLoginHandler.INVALID_USER_KEY;
        if(!AbsLoginHandler.INVALID_USER_KEY.equals(userKey)) {
            plainUserKey = new String(Base64.decode(userKey));
            plainUserKey = plainUserKey.substring(0, plainUserKey.indexOf(Constants.PASSWORD_MASK));
        }
        log.i("getPlainUserKey", "decode userKey from [%s] to [%s]", userKey, plainUserKey);
        return plainUserKey;
    }
