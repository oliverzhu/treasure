...
public class RegexValidate {
    ...
    // 用户名正则格式验证(length > 7 && length <= 50)
    private final static String USER_PATTERN_STR_EX = "^[0-9A-Za-z]{8,50}$";
    // 文件名正则格式验证(length > 0)
...
//    private final static String MOBILETELL_EXPREG_STR = "^((13[0-9])|(15[^4,\\D])|(18[0,5-9]))\\d{8}$";
    private final static String MOBILETELL_EXPREG_STR = "^1\\d{10}$";
    ...
    // 密码
    private final static String PASSWORD_STR_FIRST = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z\\[\\]{}~`!@#$%^&*()_+\\<>,.?/;:\"'|]{8,}$";
    
    // 密码 长度大于等于8位，必须含有数字、字母、特殊字符
    private final static String PASSWORD_STR_SECOND = "^(?=.*\\d)(?=.*[a-zA-Z])(?=.*[\\W\\_])\\S{8,}$";
    ...
    public static boolean isSpecialUserNameEx(String text) {
        return matchingText(USER_PATTERN_STR_EX, text);
    }
    
    public static boolean isSpecialPasswordFirst(String text) {
        return matchingText(PASSWORD_STR_FIRST, text);
    }
    public static boolean isSpecialPasswordSecond(String text) {
        return matchingText(PASSWORD_STR_SECOND, text);
    }
    /**
     * 验证是否是正整数，包含0
     * 
