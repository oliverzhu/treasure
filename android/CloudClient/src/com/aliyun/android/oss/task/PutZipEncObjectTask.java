...
public class PutZipEncObjectTask extends PutObjectTask {
    ...
    public PutZipEncObjectTask(String bucketName, String objectKey,
        ...
        super(bucketName, objectKey, contentType);
    ...
    protected HttpResponse execute() throws OSSException {
        if (isEncrypted == true && Helper.isEmptyString(new String(encryptKey))) {
            OSSException ossException = new OSSException("Encrypt key not set!");
            ossException.setErrorCode(OSSErrorCode.ENCRYPT_KEY_NOT_EMPTY);
            throw ossException;
        }
        if (isZipped) {
            try {
                ...
                ossException.setErrorCode(OSSErrorCode.ZIP_ERROR);
                throw ossException;
            }
        }
        if (isEncrypted) {
            try {
                ...
                OSSException ossException = new OSSException(e);
                ossException.setErrorCode(OSSErrorCode.ENCRYPT_ERROR);
                throw ossException;
            }
        }
        try {
