...
public class ObjectsQuery extends OSSQuery<OSSObjectSummary> {
    ...
    public List<OSSObjectSummary> getItems(PageMarker curMarker)
        ...
        GetBucketXmlObject obj;
        try {
            obj = getBucketTask.getResult();
        ...
        List<OSSObjectSummary> result = new ArrayList<OSSObjectSummary>();
        String split;
        int length = path.length();
        for (OSSObjectSummary oss: obj.getItems()) {
            if (length == 0) {
                result.add(oss);
            } else {
                split = oss.getKey().substring(0, length);
                if (split.length() >= 0) {
                    if (!split.equals(path)) {
                        result.add(oss);
                    }
                    if (oss.getKey().length() > split.length()) {
                        result.add(oss);
                    }
                }
            }
