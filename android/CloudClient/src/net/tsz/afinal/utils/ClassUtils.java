...
//            }
    ...
    public static List<Property> getPropertyList(Class<?> clazz) {
        ...
        List<Property> superList = null;
        try {
            Field[] fs = clazz.getDeclaredFields();
            String primaryKeyFieldName = getPrimaryKeyFieldName(clazz);
            Class<?> superClass = clazz.getSuperclass();
            if (superClass != Object.class && superClass != CloudObject.class) {
                superList = getPropertyList(superClass);
            }
            for (Field f : fs) {
                //必须是基本数据类型和没有标瞬时态的字段
