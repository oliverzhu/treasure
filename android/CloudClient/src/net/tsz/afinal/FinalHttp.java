...
public class FinalHttp {
    ...
    private static int socketTimeout = 30 * 1000; //超时时间，默认10秒
    private static int maxRetries = 3;//错误尝试次数，错误异常表请在RetryHandler添加
    private static int httpThreadCount = 6;//http线程池数量
    ...
    */
    ...
    public Object postSync( String url, Header[] headers, AjaxParams params, String contentType) throws IOException {
        HttpEntityEnclosingRequestBase request = new HttpPost(url);
        if(params != null) request.setEntity(paramsToEntity(params));
        if(headers != null) request.setHeaders(headers);
    ...
    public Object postSync( String url, Header[] headers, HttpEntity entity, String contentType) throws IOException {
        HttpEntityEnclosingRequestBase request = addEntityToRequestBase(new HttpPost(url), entity);
        if(headers != null) request.setHeaders(headers);
        return sendSyncRequest(httpClient, httpContext, request, contentType);
    }
  ...
  //------------------put 请求-----------------------
    ...
    public Object putSync(String url) throws IOException {
        return putSync(url, null);
    }
    
    public Object putSync( String url, AjaxParams params) throws IOException {
        return putSync(url, paramsToEntity(params),null);
    }
    
    public Object putSync(String url, HttpEntity entity, String contentType) throws IOException {
        return putSync(url,null, entity, contentType);
    }
    
    
    public Object putSync(String url,Header[] headers, HttpEntity entity, String contentType) throws IOException {
        HttpEntityEnclosingRequestBase request = addEntityToRequestBase(new HttpPut(url), entity);
        if(headers != null) request.setHeaders(headers);
        return sendSyncRequest(httpClient, httpContext, request, contentType);
    }
    ...
    public Object deleteSync(String url) throws IOException {
        return deleteSync(url,null);
    }
    
    public Object deleteSync( String url, Header[] headers) throws IOException {
        final HttpDelete delete = new HttpDelete(url);
        if(headers != null) delete.setHeaders(headers);
    ...
    protected Object sendSyncRequest(DefaultHttpClient client, HttpContext httpContext,
            HttpUriRequest uriRequest, String contentType) throws IOException {
        if(contentType != null) {
            uriRequest.addHeader("Content-Type", contentType);
        }
