...
public class BlobCache implements Closeable {
    ...
    public BlobCache(String path, int maxEntries, int maxBytes, boolean reset,
    	...
    	Log.i(TAG, "<BlobCache> start",Log.APP);
    ...
    @Override
    public void close() {
    	Log.i(TAG, "<close>",Log.APP);
        Log.d(TAG, "->syncAll",Log.APP);
        syncAll();
        Log.d(TAG, "<-syncAll",Log.APP);
        Log.d(TAG, "->closeAll",Log.APP);
        closeAll();
        Log.d(TAG, "<-closeAll",Log.APP);
    }

    private void closeAll() {
    	Log.i(TAG, "<closeAll>",Log.APP);
        mIndexBuffer = null;
        Log.d(TAG, "mapped buffer closed",Log.APP);
        closeSilently(mIndexChannel);
        Log.d(TAG, "index file channel closed",Log.APP);
        closeSilently(mIndexFile);
        Log.d(TAG, "index file closed",Log.APP);
        closeSilently(mDataFile0);
        Log.d(TAG, "data file 0 closed",Log.APP);
        closeSilently(mDataFile1);
        Log.d(TAG, "data file 1 closed",Log.APP);
    }
    ...
    private boolean loadIndex() {
    	Log.i(TAG, "<loadIndex> start",Log.APP);
        try {
            mIndexFile.seek(0);
            mDataFile0.seek(0);
            mDataFile1.seek(0);
            ...
            if (mIndexFile.read(buf) != INDEX_HEADER_SIZE) {
                Log.w(TAG, "<loadIndex> cannot read header",Log.APP);
                return false;
            }

            if (readInt(buf, IH_MAGIC) != MAGIC_INDEX_FILE) {
                Log.w(TAG, "<loadIndex> cannot read header magic",Log.APP);
                return false;
            }

            if (readInt(buf, IH_VERSION) != mVersion) {
                Log.w(TAG, "<loadIndex> version mismatch",Log.APP);
                return false;
            }
            ...
            if (checkSum(buf, 0, IH_CHECKSUM) != sum) {
                Log.w(TAG, "<loadIndex> header checksum does not match",Log.APP);
                return false;
            }

            // Sanity check
            if (mMaxEntries <= 0) {
                Log.w(TAG, "<loadIndex> invalid max entries",Log.APP);
                return false;
            }
            if (mMaxBytes <= 0) {
                Log.w(TAG, "<loadIndex> invalid max bytes",Log.APP);
                return false;
            }
            if (mActiveRegion != 0 && mActiveRegion != 1) {
                Log.w(TAG, "<loadIndex> invalid active region",Log.APP);
                return false;
            }
            if (mActiveEntries < 0 || mActiveEntries > mMaxEntries) {
                Log.w(TAG, "<loadIndex> invalid active entries",Log.APP);
                return false;
            }
            if (mActiveBytes < DATA_HEADER_SIZE || mActiveBytes > mMaxBytes) {
                Log.w(TAG, "<loadIndex> invalid active bytes",Log.APP);
                return false;
            }
            if (mIndexFile.length() !=
                    INDEX_HEADER_SIZE + mMaxEntries * 12 * 2) {
                Log.w(TAG, "<loadIndex> invalid index file length",Log.APP);
                return false;
            }
            ...
            if (mDataFile0.read(magic) != 4) {
                Log.w(TAG, "<loadIndex> cannot read data file magic",Log.APP);
                return false;
            }
            if (readInt(magic, 0) != MAGIC_DATA_FILE) {
                Log.w(TAG, "<loadIndex> invalid data file magic",Log.APP);
                return false;
            }
    ...
    private void resetCache(int maxEntries, int maxBytes) throws IOException {
    	Log.i(TAG, "<resetCache> maxEntries:" + maxEntries + " maxBytes:" + maxBytes,Log.APP);
    ...
    private void flipRegion() throws IOException {
    	Log.i(TAG, "<flipRegion> start",Log.APP);
        ...
        syncIndex();
        Log.i(TAG, "<flipRegion> end",Log.APP);
    }

    // Sync mIndexHeader to the index file.
    private void updateIndexHeader() {
    	Log.i(TAG, "<updateIndexHeader>",Log.APP);
    ...
    private void clearHash(int hashStart) {
    	Log.i(TAG, "<clearHash> hashStart:" + hashStart,Log.APP);
    ...
    public void insert(long key, byte[] data) throws IOException {
    	Log.i(TAG, "<insert> start key:" + key,Log.APP);
        ...
        updateIndexHeader();
        Log.i(TAG, "<insert> end",Log.APP);
    }

    public void clearEntry(long key) throws IOException {
    	Log.i(TAG, "<clearEntry> key:" + key,Log.APP);
        if (!lookupInternal(key, mActiveHashStart)) {
        	Log.i(TAG, "<clearEntry> nothing to clear",Log.APP);
    ...
    private void insertInternal(long key, byte[] data, int length)
            throws IOException {
    	Log.i(TAG, "<insertInternal> key:" + key +" length:" + length,Log.APP);
        byte[] header = mBlobHeader;
    ...
    public byte[] lookup(long key) throws IOException {
    	Log.i(TAG, "<lookup> key:" + key,Log.APP);
        ...
        if (lookupInternal(req.key, mInactiveHashStart)) {
            if (getBlob(mInactiveDataFile, mFileOffset, req)) {
                ...
                if (mActiveBytes + BLOB_HEADER_SIZE + req.length > mMaxBytes
                    || mActiveEntries * 2 >= mMaxEntries) {
                	Log.i(TAG, "<lookup> end not have enouth space to insert",Log.APP);
                    return true;
                }
                ...
                try {
                    ...
                    writeInt(mIndexHeader, IH_ACTIVE_ENTRIES, mActiveEntries);
                    updateIndexHeader();
                    Log.i(TAG, "<lookup> end insert success",Log.APP);
                } catch (Throwable t) {
                    Log.e(TAG, "cannot copy over",Log.APP);
                }
                return true;
            }
        }
        Log.i(TAG, "<lookup> end",Log.APP);
        return false;
    }
    ...
    private boolean getBlob(RandomAccessFile file, int offset,
    	...
    	Log.i(TAG, "<getBlob> offset:" + offset,Log.APP);
        byte[] header = mBlobHeader;
        long oldPosition = file.getFilePointer();
        try {
            file.seek(offset);
            if (file.read(header) != BLOB_HEADER_SIZE) {
                Log.w(TAG, "<getBlob> cannot read blob header",Log.APP);
                return false;
            }
            ...
            if (file.read(blob, 0, length) != length) {
                Log.w(TAG, "<getBlob> cannot read blob data",Log.APP);
                return false;
            }
            if (checkSum(blob, 0, length) != sum) {
                Log.w(TAG, "<getBlob> blob checksum does not match: " + sum,Log.APP);
                return false;
            }
            return true;
        } catch (Throwable t)  {
            Log.e(TAG, "<getBlob> failed.",Log.APP);
            return false;
        } finally {
    ...
    private boolean lookupInternal(long key, int hashStart) {
    	Log.i(TAG, "<lookupInternal> key:" + key,Log.APP);
        ...
        while (true) {
            ...
            if (candidateOffset == 0) {
                ...
                if (slot == slotBegin) {
                    Log.w(TAG, "corrupted index: clear the slot.",Log.APP);
    ...
    public void syncIndex() {
    	Log.i(TAG, "<syncIndex>",Log.APP);
        try {
            mIndexBuffer.force();
        } catch (Throwable t) {
            Log.w(TAG, "sync index failed",Log.APP);
        }
    }

    public void syncAll() {
    	Log.i(TAG, "<syncAll> start",Log.APP);
        syncIndex();
        try {
            mDataFile0.getFD().sync();
        } catch (Throwable t) {
            Log.w(TAG, "<syncAll> sync data file 0 failed",Log.APP);
        }
        try {
            mDataFile1.getFD().sync();
        } catch (Throwable t) {
            Log.w(TAG, "<syncAll> sync data file 1 failed",Log.APP);
        }
        Log.i(TAG, "<syncAll> end",Log.APP);
    }
    ...
    int getActiveCount() {
        ...
        if (count == mActiveEntries) {
        	Log.i(TAG, "<getActiveCount> count:" + count,Log.APP);
            return count;
        } else {
            Log.e(TAG, "<getActiveCount> wrong active count: " + mActiveEntries + " vs " + count,Log.APP);
    ...
    static void closeSilently(Closeable c) {
    	Log.i(TAG, "<closeSilently>",Log.APP);
        if (c == null) return;
        try {
            c.close();
        } catch (Throwable t) {
            // do nothing
            Log.e(TAG, "throwable in closeSilently(" + c + "): ",Log.APP);
        }
    }
