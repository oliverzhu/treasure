package com.client.customerservicecenter.widget.scroll;
...
public class GridViewHelper {
			...
			ViewGroup group,List datas,Class clazz)
	{
		...
		int itemWidth = (int) (screenWidth/columns * Constants.THRESHOLD_GRID_ITEM);
		...
		for(int i = 0;i<childCount;i++) {
			...
			try {
				...
				adapter.setTotalDataSize(datas.size());
				adapter.setView(item);
				adapter.setDatasourceChangeListener(mDatasourceChangeListener);
			} catch (Exception e) {
				e.printStackTrace();
			} 
