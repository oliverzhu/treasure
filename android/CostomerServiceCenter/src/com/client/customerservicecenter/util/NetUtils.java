...
public class NetUtils {
	...
	public static Bitmap getbitmap(String imageUri) {
		Log.v(TAG, "getbitmap:" + imageUri,Log.APP);
		// 显示网络上的图片
		Bitmap bitmap = null;
		try {
			...
			Log.v(TAG, "image download finished." + imageUri,Log.APP);
		} catch (IOException e) {
			e.printStackTrace();
			Log.v(TAG, "getbitmap bmp fail---",Log.APP);
			return null;
		}
		return bitmap;
	}
	 ...
	 * @return
	 */
	public static JSONArray getJSONArrayByGet(String uri) {
	...
	public static JSONObject uploadParamsByPost(String actionUrl, String query) {
		...
		try {
			...
			// query is your body
			conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");// 请求�?
																							// 必须设置
			conn.setRequestProperty("Content-Length", query.toString().getBytes("UTF-8").length + "");// 注意是字节长�?
																										// 不是字符长度
