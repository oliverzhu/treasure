        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs
{
    ...
    //add by zjw
    private static boolean isArtistPinyinColumnExist = false;
...
*/   
    ...
    public void init(Cursor c) {
        ...
        if (mArtistCursor == null) {
            Log.w(TAG, "mArtistCursor is null",Log.APP);
            ...
            case NEW_PLAYLIST:
                if (resultCode == RESULT_OK) {
                    ...
                    if (uri != null && selectItemString != null) {
                        ...
                        Log.i(TAG, "<onActivityResult> selectItemId = " + selectItemId,Log.APP);
    ...
    private Cursor getArtistCursor(AsyncQueryHandler async, String filter) {
    	...
    	//update by zjw
    	//MediaStore.Audio.Artists.ARTIST_PINYIN_KEY -->artist_pinyin_key
        String sortOrder = MediaStore.Audio.Artists.DEFAULT_SORT_ORDER;
        if(isArtistPinyinColumnExist)
        {
        	sortOrder = "artist_pinyin_key";
        }
    ...
    static class ArtistAlbumListAdapter extends SimpleCursorTreeAdapter implements SectionIndexer {
        ...
        private void getColumnIndices(Cursor cursor) {
            if (cursor != null) {
                ...
                if(isArtistPinyinColumnExist)
                {
                	mGroupArtistPinyinIdx = cursor.getColumnIndexOrThrow("artist_pinyin_key");
                }
        ...
        private int getGroupPositon(int position) {
            ...
            if (listview == null) {
                Log.e(TAG, "getGroupPositon with ExpandableListView is null",Log.APP);
                return 0;
            }
    ...
    {
    	//add by zjw
    	isArtistPinyinColumnExist = MusicUtils.isArtistNamePinyinKeyExist();
    	ArrayList<String> mCursorColsList = new ArrayList<String>();
        mCursorColsList.add(MediaStore.Audio.Albums._ID);
        mCursorColsList.add(MediaStore.Audio.Artists.ARTIST);
        mCursorColsList.add(MediaStore.Audio.Artists.NUMBER_OF_ALBUMS);
        mCursorColsList.add(MediaStore.Audio.Artists.NUMBER_OF_TRACKS);
        if(isArtistPinyinColumnExist)
        {
