...
import com.custom.music.view.TouchInterceptor;
...
//    private OmaDrmClient mDrmClient = null;
    ...
    private static boolean mWithtabs = false;
  ...
  //add by zjw
    private static boolean isTilePinyinColumnExist = false;
    
    private TextView trackActionBarTitle;
...
//        requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
        ...
        if (intent != null) {
            ...
            if (mWithtabs) {
                ...
                ViewGroup view = (ViewGroup)LayoutInflater.from(this).inflate(R.layout.track_action_bar, null);
                actionBar.setDisplayOptions(ActionBar.DISPLAY_SHOW_CUSTOM, ActionBar.DISPLAY_SHOW_CUSTOM | ActionBar.DISPLAY_SHOW_HOME |
                        ActionBar.DISPLAY_SHOW_TITLE);
                actionBar.setCustomView(view, new ActionBar.LayoutParams(ActionBar.LayoutParams.FILL_PARENT, 
                                                                         ActionBar.LayoutParams.FILL_PARENT,
                                                                         Gravity.CENTER_VERTICAL | Gravity.CENTER));
                LinearLayout trackBack = (LinearLayout) view.findViewById(R.id.track_back);
                trackBack.setOnClickListener(new OnClickListener() {
                ...
                trackActionBarTitle = (TextView) view.findViewById(R.id.track_name);
            /// @}
            }
        }
        ...
        if (icicle != null) {
            ...
            //update by zjw
            mEditMode = false;
            		//intent.getAction() == null ? false : intent.getAction().equals(Intent.ACTION_EDIT);
        }
        
        //update by zjw
        //MediaStore.Audio.Media.IS_DRM -->is_drm
        //MediaStore.Audio.Media.DRM_METHOD --> drm_method
        //MediaStore.Audio.Media.TITLE_PINYIN_KEY -->title_pinyin_key
        initCursorColumns();
...
//        }
    ...
    {
    	Log.i(TAG, "<onServiceConnected>",Log.APP);
    ...
    public void onServiceDisconnected(ComponentName name) {
    	Log.i(TAG, "<onServiceDisconnected>",Log.APP);
    ...
    public void onResume() {
        ...
        if (mTrackCursor != null) {
            ...
            if (mEditMode) {
                Log.i(TAG, "need to requery data!",Log.APP);
    ...
    public void refreshEmptyView() {
    	Log.i(TAG, "<refreshEmptyView>",Log.APP);
        if (getListView().getAdapter() != null
                && getListView().getAdapter().getCount() == 0
                && getListView().getCount() != 0) {
            Log.i(TAG, "getExpandableListView().getCount() = "
                    + getListView().getCount(),Log.DATA);
    ...
    private BroadcastReceiver mScanListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i(TAG, "mScanListener.onReceive:" + action + ", status = " + status,Log.APP);
            if (Intent.ACTION_MEDIA_SCANNER_STARTED.equals(action) ||
                    Intent.ACTION_MEDIA_SCANNER_FINISHED.equals(action)) {
            	Log.i(TAG, "<mScanListener.onReceive> ACTION_MEDIA_SCANNER_STARTED || ACTION_MEDIA_SCANNER_FINISHED",Log.APP);
                mReScanHandler.sendEmptyMessage(0);
            } else if (Intent.ACTION_MEDIA_UNMOUNTED.equals(action)) {
            	Log.i(TAG, "<mScanListener.onReceive> ACTION_MEDIA_UNMOUNTED",Log.APP);
            	...
            	Log.i(TAG, "<mScanListener.onReceive> ACTION_MEDIA_MOUNTED",Log.APP);
    ...
    private Handler mReScanHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
        	Log.i(TAG, "<mReScanHandler.handleMessage>",Log.APP);
            if (mAdapter != null) {
                getTrackCursor(mAdapter.getQueryHandler(), null, true);
            }
    ...
    public Handler mListHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
        	Log.i(TAG, "<mListHandler.handleMessage>",Log.APP);
            refreshEmptyView();
            }
    };
    ...
    public void init(Cursor newCursor, boolean isLimited) {
    	Log.i(TAG, ">>> init isLimited:" + isLimited,Log.APP);
    ...
    private void setAlbumArtBackground() {
        ...
        Log.i(TAG, "<setAlbumArtBackground> mAlbumId = " + mAlbumId + ", mAlbumArtBitmap = " + mAlbumArtBitmap,Log.APP);
        ...
        try {
            ...
            Log.e(TAG, "Exception while fetching album art!! " + ex,Log.APP);
        }
    }

    private void setTitle() {
    	Log.i(TAG, ">>> setTitle",Log.APP);
        ...
        if (!mWithtabs) {
            if (fancyName != null) {
                setTitle(fancyName);
                if(trackActionBarTitle != null)
                {
                	trackActionBarTitle.setText(fancyName);
                }
            } else {
            	if(trackActionBarTitle != null)
                {
                	trackActionBarTitle.setText(R.string.tracks_title);
                }
    ...
    private TouchInterceptor.DropListener mDropListener =
        ...
        public void drop(int from, int to) {
            if (mTrackCursor instanceof NowPlayingCursor) {
                ...
                Log.i(TAG, "<drop> from = " + from + ", to = " + to + ", isSuccesss = " + isSuccesss,Log.APP);
                /// @}
            }
        }
    };
    ...
    private TouchInterceptor.RemoveListener mRemoveListener =
        ...
        public void remove(int which) {
        	Log.i(TAG, "<remove> which:" + which,Log.APP);
            removePlaylistItem(which);
        }
    };

    private void removePlaylistItem(int which) {
    	Log.i(TAG, "<removePlaylistItem> which:" + which,Log.APP);
        View v = mTrackList.getChildAt(which - mTrackList.getFirstVisiblePosition());
        if (v == null) {
            Log.i(TAG, "No view when removing playlist item " + which,Log.APP);
            return;
        }
        try {
    ...
    private BroadcastReceiver mNowPlayingListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(MediaPlaybackService.META_CHANGED)) {
            	Log.i(TAG, "<mNowPlayingListener.onReceive> META_CHANGED",Log.APP);
            	MusicUtils.updateNowPlaying(TrackBrowserActivity.this, mOrientation);
                getListView().invalidateViews();
            } else if (intent.getAction().equals(MediaPlaybackService.QUEUE_CHANGED)) {
            	Log.i(TAG, "<mNowPlayingListener.onReceive> QUEUE_CHANGED",Log.APP);
                if (mDeletedOneRow) {
    ...
    private boolean isMusic(Cursor c) {
        ...
        Log.i(TAG, "<isMusic> ismusic" + ismusic,Log.APP);
        return ismusic;
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfoIn) {
        ...
        //update by zjw
        //MusicFeatureOption.IS_SUPPORT_DRM --> DrmUtils.isSupportDrm()
        //MediaStore.Audio.Media.IS_DRM --> is_drm
        //MediaStore.Audio.Media.DRM_METHOD --> drm_method
        if (DrmUtils.isSupportDrm()) {
            isDrm = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow("is_drm"));
            int drmMethod = mTrackCursor.getInt(mTrackCursor.getColumnIndexOrThrow("drm_method"));
        ...
        // information. @{
        //update by zjw
        //com.mediatek.internal.R.string.drm_protectioninfo_title --> Protection Info
        if (DrmUtils.isSupportDrm() && isDrm == 1) {
            menu.add(0, DRM_INFO, 0, "Protection Info");
        }
        /// @}
    }
    ...
    public boolean onContextItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case DRM_INFO:
            	...
            	//MusicFeatureOption.IS_SUPPORT_DRM --> DrmUtils.isSupportDrm()
                if (DrmUtils.isSupportDrm()) {
                	DrmUtils.showProtectionInfoDialog(this,mSelectedId);
                }
                return true;
        ...
        if (mPlaylist != null && !mPlaylist.equals("recentlyadded") && curpos >= 0 &&
            ...
            switch (event.getKeyCode()) {
                case KeyEvent.KEYCODE_DPAD_UP:
                	Log.i(TAG, "<dispatchKeyEvent> KEYCODE_DPAD_UP",Log.APP);
                    moveItem(true);
                    return true;
                case KeyEvent.KEYCODE_DPAD_DOWN:
                	Log.i(TAG, "<dispatchKeyEvent> KEYCODE_DPAD_DOWN",Log.APP);
                    moveItem(false);
                    return true;
                case KeyEvent.KEYCODE_DEL:
                	Log.i(TAG, "<dispatchKeyEvent> KEYCODE_DEL",Log.APP);
                    removeItem();
                    return true;
            }
        }
    ...
    private void moveItem(boolean up) {
    	Log.i(TAG, "<moveItem> up:" + up,Log.APP);
        int curcount = mTrackCursor.getCount(); 
        int curpos = mTrackList.getSelectedItemPosition();
    ...
    {
        Log.i(TAG, "<onListItemClick> play song start ["
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        /// M: set party shufll,shuffle all,save as playlist invisble at SoundRecord,FM,Phone Recording playlist @{
        if (mPlayListName != null
                && !mPlayListName.equals("")
                && (mPlayListName.equals(getText(R.string.soundrecord_playlist_name))
                        || mPlayListName.equals(getText(R.string.fmrecord_playlist_name)) || mPlayListName
        ...
        if ((findViewById(R.id.sd_icon).getVisibility()) == View.VISIBLE) {
            Log.i(TAG, "SDcard not ready, disable option menu!",Log.APP);
            return false;
        }
        /// @}
        return true;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        switch (requestCode) {
            case SCAN_DONE:
            	Log.i(TAG, "<onActivityResult> SCAN_DONE",Log.APP);
            ...
            case NEW_PLAYLIST:
            	Log.i(TAG, "<onActivityResult> NEW_PLAYLIST",Log.APP);
                if (resultCode == RESULT_OK) {
                    ...
                    Log.i(TAG, "onActivityResult: selectAudioId = " + selectAudioId,Log.APP);
            ...
            case SAVE_AS_PLAYLIST:
            	Log.i(TAG, "<onActivityResult> SAVE_AS_PLAYLIST",Log.APP);
                if (resultCode == RESULT_OK) {
                    Uri uri = intent.getData();
                    /// M: Return SAVE_PLAYLIST_FLAG only when overwrite playlist by save as playlist
            ...
            case ADD_SONG:
            	Log.i(TAG, "<onActivityResult> ADD_SONG",Log.APP);
            ...
            case ADD_FOLDER_TO_PLAY:
            	Log.i(TAG, "<onActivityResult> ADD_FOLDER_TO_PLAY",Log.APP);
                if (intent != null) {
            ...
            case ADD_SONG_TO_PLAY:
            	Log.i(TAG, "<onActivityResult> ADD_SONG_TO_PLAY",Log.APP);
                if (intent != null && intent.getData() != null) {
    ...
    private Cursor getTrackCursor(TrackListAdapter.TrackQueryHandler queryhandler, String filter,
            boolean async) {
    	Log.i(TAG, "<getTrackCursor> async:" + async,Log.APP);
        ...
        if (mGenre != null) {
            ...
            if (mPlaylist.equals("nowplaying")) {
                ...
                if(isTilePinyinColumnExist)
                {
                	mSortOrder = "title_pinyin_key";
                }
                ret = queryhandler.doQuery(uri,
    ...
    private class NowPlayingCursor extends AbstractCursor
    {
        ...
        private void makeNowPlayingCursor() {
        	Log.i(TAG, "<makeNowPlayingCursor>",Log.APP);
    ...
    static class TrackListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        class TrackQueryHandler extends AsyncQueryHandler {
            ...
            public Cursor doQuery(Uri uri, String[] projection,
            	...
            	Log.i(TAG, "<doQuery> async:" + async,Log.APP);
                if (async) {
                    // Get 100 results first, which is enough to allow the user to start scrolling,
                    // while still being very fast.
                    Uri limituri = uri.buildUpon().appendQueryParameter("limit", "10").build();
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                Log.i(TAG, "<onQueryComplete> count is " + (cursor == null ? null : cursor.getCount())
                        + "   " + mActivity,Log.APP);
                mActivity.init(cursor, cookie != null);
                /// M: Cursor may be close when activity has been finishing, so we need check it.
                if (token == 0 && cookie != null && cursor != null && !cursor.isClosed() && cursor.getCount() >= 10) {
        ...
        private void getColumnIndices(Cursor cursor) {
        	Log.i(TAG, "<getColumnIndices>",Log.APP);
            if (cursor != null) {
                ...
                if(isTilePinyinColumnExist)
                {
                	mTitlePinyinIdx = cursor.getColumnIndexOrThrow("title_pinyin_key");
                }
                ...
                //MediaStore.Audio.Media.DRM_METHOD --> drm_method
                if (DrmUtils.isSupportDrm()) {
                    mIsDrmIdx = cursor.getColumnIndexOrThrow("is_drm");
                    mDrmMethodIdx = cursor.getColumnIndexOrThrow("drm_method");
                }
                /// @}
                if (mIndexer != null) {
                    mIndexer.setCursor(cursor);
                } else if (!mActivity.mEditMode && mActivity.mAlbumId == null && isTilePinyinColumnExist) {
        ...
        public View newView(Context context, Cursor cursor, ViewGroup parent) {
        	...
        	vh.line1.setTextAppearance(context, R.style.TextAppearance_primary_title_light);
            
            vh.line2 = (TextView) v.findViewById(R.id.line2);
        	vh.line2.setTextAppearance(context, R.style.TextAppearance_secondary_title_light);
        ...
        public void changeCursor(Cursor cursor) {
        	Log.i(TAG, "<changeCursor>",Log.APP);
        ...
        public Cursor runQueryOnBackgroundThread(CharSequence constraint) {
        	Log.i(TAG, "<runQueryOnBackgroundThread> constraint:" + constraint,Log.APP);
        ...
        private void updateDrmLockIcon(ImageView drmLock, Cursor cursor) {
        	Log.i(TAG, "<updateDrmLockIcon>",Log.APP);
            ...
            if (DrmUtils.isSupportDrm()) {
                ...
                if (isDrm == 1 && drmMethod != DrmUtils.DRMMETHOD_METHOD_FL) {
                    ...
                    } else if (rightsStatus == DrmUtils.RIGHTSSTATUS_RIGHTS_VALID) {
                    	//update by zjw
                    	//com.mediatek.internal.R.drawable.drm_green_lock --> R.drawable.drm_green_lock
                        drmLock.setImageResource(R.drawable.drm_green_lock);
                        drmLockVisible = View.VISIBLE;
                    } else {
                    	//update by zjw
                    	//com.mediatek.internal.R.drawable.drm_red_lock --> 
                        drmLock.setImageResource(R.drawable.drm_red_lock);
                        drmLockVisible = View.VISIBLE;
                    }
                } 
            }
        ...
        public void reloadStringOnLocaleChanges() {
        	Log.i(TAG, "<reloadStringOnLocaleChanges>",Log.APP);
    ...
    private void initAdapter() {
    	Log.i(TAG, ">>> initAdapter",Log.APP);
        mAdapter = (TrackListAdapter) getLastNonConfigurationInstance();
        if (mAdapter == null) {
            Log.i(TAG, "<initAdapter> starting query",Log.APP);
    ...
    private void addSongToPlaylist(Uri uri) {
    	Log.i(TAG, ">>> addSongToPlaylist",Log.APP);
        ...
        Log.i(TAG, "addSongToPlaylist: data=" + data,Log.APP);
        ...
        if (selectFileId < 0) {
            Log.w(TAG, "addFileToPlay: select file is not audio file!",Log.APP);
    ...
    private void addFolderToPlay(String folderPath) {
    	Log.i(TAG, "<addFolderToPlay> folderPath:" + folderPath,Log.APP);
    ...
    private void addSongToPlay(Uri uri) {
        ...
        /// M: to avoid the JE when query the file whoes name contains "'"
        /// which is the escape character of SQL.
        Log.i(TAG, "<addSongToPlay> data=" + data,Log.APP);
        data = data.replaceAll("'", "''");
        data = data.replaceFirst("file://", "");
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if ((findViewById(R.id.sd_icon).getVisibility()) == View.VISIBLE) {
            Log.i(TAG, "Configuration Changed at database error, return!",Log.APP);
            return;
        }
        ...
        if (mOrientation == Configuration.ORIENTATION_PORTRAIT && mIsInBackgroud) {
            ...
            Log.i(TAG, "onConfigurationChanged clear background album art when in background.",Log.APP);
        }
    ...
    private class AlbumArtFetcher extends AsyncTask<Long, Void, Bitmap> {
        protected Bitmap doInBackground(Long... albumId) {
            ...
            if (mAlbumArtBitmap == null) {
                ...
                Log.i(TAG, "AlbumArtFetcher: getArtwork returns " + mAlbumArtBitmap,Log.APP);
            ...
            if (mTrackCursor instanceof NowPlayingCursor) {
                if (MusicUtils.sService != null) {
                    try {
                        ...
                        Log.e(TAG, "RemoteException when setQueuePosition: " + ex, Log.APP);
                    }
                }
            } else {
    ...
    private int getDrmRightsStatus(Cursor cursor, boolean isForTap) {
        ...
        try {
            if (isForTap) {
                ...
                rightsStatus = DrmUtils.checkRightsStatus(this, uri);
            }
        } catch (IllegalArgumentException e) {
            Log.e(TAG, "getDrmRightsStatus throw IllegalArgumentException " + e,Log.APP);
        }
        
        Log.i(TAG, "<getDrmRightsStatus> rightsStatus=" + rightsStatus,Log.APP);
        return rightsStatus;
    }
     ...
     */
    private boolean checkDrmRightsForPlay(Cursor cursor, int position, boolean isNowplaying, boolean showFlInvalidToast) {
    	//update by zjw
    	//MusicFeatureOption.IS_SUPPORT_DRM --> DrmUtils.isSupportDrm()
        if (!DrmUtils.isSupportDrm()) {
            return true;
        ...
        Log.i(TAG, "checkDrmRightsForPlay: isDrm=" + isDrm,Log.APP);
        try {
            if (isDrm == 0) {
                return true;
            }
            ...
            Log.i(TAG, "<checkDrmRightsForPlay> drmMethod=" + drmMethod,Log.APP);
            ...
            switch (rightsStatus) {
                case DrmUtils.RIGHTSSTATUS_RIGHTS_VALID:
                    ...
                    if (drmMethod == DrmUtils.DRMMETHOD_METHOD_FL) {
                        // if FL, play directly
                        return true;
                    }
                    /// @}
                    ...
                    if (!isNowplaying && MusicUtils.sService != null) {
                        ...
                        Log.i(TAG, "curPos=" + curPos + ", position=" + position,Log.APP);
                ...
                case DrmUtils.RIGHTSSTATUS_RIGHTS_INVALID:
                    Log.i(TAG, "OmaDrmStore.RightsStatus.RIGHTS_INVALID=",Log.APP);
                    ...
                    /// @}
                    DrmUtils.showRefreshLicenseDialog(this,getUri(cursor));
                    break;
                case DrmUtils.RIGHTSSTATUS_SECURE_TIMER_INVALID:
                	DrmUtils.showSecureTimerInvalidDialog(this);
                    break;
                default:
                    Log.e(TAG, "No such rights status for current DRM file!!",Log.APP);
                    break;
            }
            return false;
        } catch (RemoteException re) {
            Log.e(TAG, "RemoteException in service call!",Log.APP);
            return false;
        } finally {
    ...
    private boolean canDispalyRingtone(int isDrm, int drmMethod) {
        ...
        //update by zjw
        if (isDrm == 1 && drmMethod == DrmUtils.DRMMETHOD_METHOD_FL) {
            return checkDrmRightsForPlay(mTrackCursor, mSelectedPosition, false, false);
        }
        return false;
    }
    ...
    {
    	//add by zjw
        isTilePinyinColumnExist = MusicUtils.isMediaTitlePinyinKeyExist();
    	ArrayList<String> mCursorColsList = new ArrayList<String>();
        mCursorColsList.add(MediaStore.Audio.Media._ID);
        mCursorColsList.add(MediaStore.Audio.Media.TITLE);
        mCursorColsList.add(MediaStore.Audio.Media.DATA);
        mCursorColsList.add(MediaStore.Audio.Media.ALBUM);
        mCursorColsList.add(MediaStore.Audio.Media.ARTIST);
        mCursorColsList.add(MediaStore.Audio.Media.ARTIST_ID);
        mCursorColsList.add(MediaStore.Audio.Media.DURATION);
        
        ArrayList<String> mPlaylistMemberColsList = new ArrayList<String>();
        mPlaylistMemberColsList.add(MediaStore.Audio.Playlists.Members._ID);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.TITLE);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.DATA);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.ALBUM);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.ARTIST);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.ARTIST_ID);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.DURATION);
        mPlaylistMemberColsList.add(MediaStore.Audio.Playlists.Members.PLAY_ORDER);
        mPlaylistMemberColsList.add(MediaStore.Audio.Playlists.Members.AUDIO_ID);
        mPlaylistMemberColsList.add(MediaStore.Audio.Media.IS_MUSIC);
        ...
        if(DrmUtils.isSupportDrm())
        {
        	...
        	mPlaylistMemberColsList.add("is_drm");
        	mPlaylistMemberColsList.add("drm_method");
        }
        
        if(isTilePinyinColumnExist)
        {
        	mCursorColsList.add("title_pinyin_key");
        	mPlaylistMemberColsList.add("title_pinyin_key");
        }
        
        mCursorCols = (String[])mCursorColsList.toArray(new String[1]);
        mPlaylistMemberCols = (String[])mPlaylistMemberColsList.toArray(new String[1]);
    }
}
