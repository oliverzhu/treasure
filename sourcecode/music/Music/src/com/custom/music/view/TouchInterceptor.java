...
public class TouchInterceptor extends ListView {
    ...
    private final ContentObserver mContentObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            ...
            mIsDraw = true;
            Log.i(TAG, "onChange: mIsDraw = " + mIsDraw,Log.APP);
        }
    };
    /**
    ...
    OnPreDrawListener mPreDrawListener = new OnPreDrawListener() {
        ...
        public boolean onPreDraw() {
            Log.i(TAG, "onPreDraw: mIsDraw = " + mIsDraw,Log.APP);
            return mIsDraw;
        }
    };
    ...
    private void startDragging(Bitmap bm, int x, int y) {
        Log.i(TAG, "startDragging ",Log.APP);
        stopDragging();
    ...
    private void stopDragging() {
        Log.i(TAG, "stopDragging() ",Log.APP);
        if (mDragView != null) {
        ...
        if ((mListviewWidth == oldw) && (mListViewHeight == oldh)) {
            ...
            synchronized (mSizeChangedLock) {
                if (mUpgradeAlbumArtListener != null
                    ...
                    Log.i(TAG, "onSizeChanged with right size, call upgrade album art:orientation=" + orientation,Log.APP);
                }
    ...
    public void waitMeasureFinished(boolean isLandscape) {
        if (isLandscape ? (mListviewWidth > mListViewHeight) : (mListviewWidth < mListViewHeight)) {
            ...
            Log.i(TAG, "waitMeasureFinished: " + "(w" + mListviewWidth + ",h"
                    + mListViewHeight + ")",Log.APP);
            return;
        } 
        ...
        synchronized (mSizeChangedLock) {
            ...
            while (!(isLandscape ? (mListviewWidth > mListViewHeight)
                ...
                try {
                    ...
                    Log.i(TAG, "Wait for listview onMeasure finished!" + time,Log.APP);
                } catch (InterruptedException e) {
                    Log.e(TAG, "wait has been interupted " + e,Log.APP);
                }
            }
        }
        Log.i(TAG, "waitMeasureFinished: listview has finished measure!",Log.APP);
    }
    ...
    public void registerContentObserver(Context context) {
        context.getContentResolver().registerContentObserver(
         ...
         Log.i(TAG, "registerContentObserver " + mContentObserver,Log.APP);
    }
    ...
    public void unregisterContentObserver(Context context) {
        ...
        Log.i(TAG, "unregisterContentObserver " + mContentObserver,Log.APP);
    }
