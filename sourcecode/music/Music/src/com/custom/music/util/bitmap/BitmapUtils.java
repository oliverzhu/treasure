...
public class BitmapUtils {
	...
	public static Bitmap resizeAndCropCenter(String filePath,int targetWidth, int targetHeight,boolean recycle) {
		...
		if (recycle) sampleBitmap.recycle();
		return targetBitmap;
	}
	...
	public static Bitmap resizeAndCropCenter(Bitmap bitmap,int targetWidth, int targetHeight,boolean recycle) {
		...
		int scaleWidth = w / targetWidth;
		int scaleHeight = h / targetHeight;
		int scale = scaleWidth < scaleHeight ? scaleWidth : scaleHeight;
		if (scale < 1) {
			scale = 1;
		}
		//get scalebitmap
		float fScaleWidth = targetWidth / ((float) w);
		float fScaleHeight = targetHeight / ((float) h);
		float fScale = fScaleWidth > fScaleHeight ? fScaleWidth : fScaleHeight;
		if (fScale > 1)
			fScale = 1;
		Matrix matrix = new Matrix();
		...
		if(bitmapX == 0 && bitmapY == 0 && targetWidth == w && targetHeight == h)
		{
			return bitmap;
		}
		Bitmap targetBitmap = Bitmap.createBitmap(scaleBitmap, bitmapX,
