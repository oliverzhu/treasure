...
import com.custom.music.AlbumBrowserActivity.AlbumListAdapter.ViewHolder;
    ...
    implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
  ...
  //add by zjw
    private static boolean isAlbumPinyinColumnExist = false;
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (mAdapter == null) {
            Log.i(TAG, "starting query",Log.APP);
    ...
    private BroadcastReceiver mScanListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// @}
            Log.i(TAG, "<mScanListener.onReceive> action:" + action + ", status = " + status,Log.APP);
    ...
    public void init(Cursor c) {
        ...
        if (mAlbumCursor == null) {
            Log.w(TAG, "mAlbumCursor is null",Log.APP);
        ...
        switch (requestCode) {
            ...
            case NEW_PLAYLIST:
                if (resultCode == RESULT_OK) {
                    ...
                    Log.i(TAG, "<onActivityResult> selectAlbumId = " + selectAlbumId,Log.APP);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mAlbumCursor == null) {
            Log.w(TAG, "Album cursor is null, need not show option menu!",Log.APP);
            return false;
        }
        ...
        if(mQueryText != null && !mQueryText.toString().equals("")){
            Log.e(TAG, "setQueryText:" + mQueryText,Log.APP);
    ...
    private Cursor getAlbumCursor(AsyncQueryHandler async, String filter) {
        ...
        /// M: add for chinese sorting
    	//update by zjw
    	//MediaStore.Audio.Albums.ALBUM_PINYIN_KEY --> album_pinyin_key
        String sortOrder = MediaStore.Audio.Albums.DEFAULT_SORT_ORDER;
        if(isAlbumPinyinColumnExist)
        {
        	sortOrder = "album_pinyin_key";
        }
        Cursor ret = null;
        if (mArtistId != null) {
    ...
    static class AlbumListAdapter extends SimpleCursorAdapter implements SectionIndexer {
        ...
        class QueryHandler extends AsyncQueryHandler {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                ...
                Log.i(TAG, "<onQueryComplete> "
                        + (cursor == null ? null : cursor.getCount()),Log.APP);
        ...
        private void getColumnIndices(Cursor cursor) {
            if (cursor != null) {
                ...
                if(isAlbumPinyinColumnExist)
                {
                	mAlbumPinyinIdx = cursor.getColumnIndexOrThrow("album_pinyin_key");
                }
        ...
        void quitLazyLoadingThread() {
            ...
            Log.i(TAG, "Quit lazy loading thread when activity ondestroy: isQuitSuccuss = " + isQuitSuccuss,Log.APP);
        }
    }
    ...
    /**
     * When input the keyword of search ,can't let the optionmenu showed
     */
    @SuppressLint("NewApi")
	@Override
    ...
    {
    	//add by zjw
    	isAlbumPinyinColumnExist = MusicUtils.isAlbumNamePinyinKeyExist();
    	ArrayList<String> mCursorColsList = new ArrayList<String>();
        mCursorColsList.add(MediaStore.Audio.Albums._ID);
        mCursorColsList.add(MediaStore.Audio.Albums.ARTIST);
        mCursorColsList.add(MediaStore.Audio.Albums.ALBUM);
        mCursorColsList.add(MediaStore.Audio.Albums.ALBUM_ART);
        if(isAlbumPinyinColumnExist)
        {
