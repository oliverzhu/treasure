...
package com.custom.music.bluetooth.avcrp;
...
public class ServiceAvrcpStub extends IBTAvrcpMusic.Stub {
    ...
    public void registerCallback(IBTAvrcpMusicCallback callback) {
        ...
        Log.i(TAG, "[AVRCP] ServiceAvrcpStub. registerCallback",Log.APP);
        if (callback != null) {
    ...
    public void unregisterCallback(IBTAvrcpMusicCallback callback) {
        Log.i(TAG, "[AVRCP] ServiceAvrcpStub. unregisterCallback",Log.APP);
        if (callback != null) {
    ...
    public boolean regNotificationEvent(byte eventId, int interval) {
        switch (eventId) {
            case 0x01: // playstatus
                bPlaybackFlag = true;
                Log.i(TAG, "[AVRCP] bPlaybackFlag flag is " + bPlaybackFlag,Log.APP);
                return true;
            case 0x02: // track change
                bTrackchangeFlag = true;
                Log.i(TAG, "[AVRCP] bTrackchange flag is " + bTrackchangeFlag,Log.APP);
                return bTrackchangeFlag;
            case 0x09: // playing content
                bTrackNowPlayingChangedFlag = true;
                return true;
            default:
                Log.e(TAG, "[AVRCP] MusicApp doesn't support eventId:" + eventId,Log.APP);
                break;
        }
    ...
    public boolean setShuffleMode(int shufflemode) {
        ...
        Log.i(TAG, "[AVRCP] setShuffleMode music_mode:" + mode,Log.APP);
    ...
    public boolean setRepeatMode(int repeatmode) {
        ...
        Log.i(TAG, String.format("[AVRCP] setRepeatMode musid_mode:%d", mode),Log.APP);
    ...
    public void notifyBTAvrcp(String s) {
        ...
        Log.i(TAG, "[AVRCP] notifyBTAvrcp " + s,Log.APP);
    ...
    protected void notifyPlaybackStatus(byte status) {
        ...
        Log.i(TAG, "[AVRCP] notifyPlaybackStatus " + status + " N= " + N,Log.APP);
        for (int i = 0; i < N; i++) {
    ...
    protected void notifyTrackChanged() {
        ...
        final int N = mAvrcpCallbacksList.beginBroadcast();
        Log.i(TAG, "[AVRCP] notifyTrackChanged " + " N= " + N,Log.APP);
        for (int i = 0; i < N; i++) {
            IBTAvrcpMusicCallback listener = mAvrcpCallbacksList.getBroadcastItem(i);
            try {
    ...
    protected void notifyNowPlayingContentChanged() {
        Log.i(TAG, "[AVRCP] notifyNowPlayingContentChanged ",Log.APP);
        ...
        Log.i(TAG, "[AVRCP] notifyNowPlayingContentChanged " + " N= " + N,Log.APP);
    ...
    protected void notifyVolumehanged(byte volume) {
        Log.i(TAG, "[AVRCP] notifyVolumehanged " + volume,Log.APP);
    }
}
