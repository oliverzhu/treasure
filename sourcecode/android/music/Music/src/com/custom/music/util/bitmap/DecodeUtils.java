...
package com.custom.music.util.bitmap;
...
import com.custom.music.util.thread.ThreadPool.CancelListener;
...
public class DecodeUtils {
    ...
    public static void decodeBounds(JobContext jc, FileDescriptor fd,
            Options options) {
    	Log.i(TAG, "<decodeBounds> FileDescriptor:" + fd, Log.APP);
    ...
    public static Bitmap decode(JobContext jc, byte[] bytes, int offset,
    	...
    	Log.i(TAG, "<decode> byte.length:" + length, Log.APP);
        if (options == null) options = new Options();
        jc.setCancelListener(new DecodeCanceller(options));
        setOptionsMutable(options);
        return ensureGLCompatibleBitmap(
    ...
    public static void decodeBounds(JobContext jc, byte[] bytes, int offset,
            int length, Options options) {
    	Log.i(TAG, "<decodeBounds> byte.length:" + length, Log.APP);
        Utils.assertTrue(options != null);
        options.inJustDecodeBounds = true;
        jc.setCancelListener(new DecodeCanceller(options));
        BitmapFactory.decodeByteArray(bytes, offset, length, options);
    ...
    public static Bitmap decodeThumbnail(
    	...
    	Log.i(TAG, "<decodeThumbnail> filePath:" + filePath, Log.APP);
    ...
    public static Bitmap decodeThumbnail(
    	...
    	Log.i(TAG, "<decodeThumbnail> FileDescriptor:" + fd, Log.APP);
        if (options == null) options = new Options();
        jc.setCancelListener(new DecodeCanceller(options));
        ...
        float scale = (float) targetSize / (type == Constants.TYPE_MICROTHUMBNAIL
    ...
    public static Bitmap decodeIfBigEnough(JobContext jc, byte[] data,
    	...
    	Log.i(TAG, "<decodeIfBigEnough>", Log.APP);
        if (options == null) options = new Options();
        jc.setCancelListener(new DecodeCanceller(options));
    ...
    public static BitmapRegionDecoder createBitmapRegionDecoder(
            ...
            boolean shareable) {
    	Log.i(TAG, "<createBitmapRegionDecoder> length:" + length, Log.APP);
    ...
    public static BitmapRegionDecoder createBitmapRegionDecoder(
    	...
    	Log.i(TAG, "<createBitmapRegionDecoder> filePath:" + filePath, Log.APP);
        try {
    ...
    public static BitmapRegionDecoder createBitmapRegionDecoder(
    	...
    	Log.i(TAG, "<createBitmapRegionDecoder> FileDescriptor:" + fd, Log.APP);
        try {
    ...
    public static BitmapRegionDecoder createBitmapRegionDecoder(
    	...
    	Log.i(TAG, "<createBitmapRegionDecoder> InputStream", Log.APP);
        try {
    ...
    public static Bitmap decode(JobContext jc, byte[] data, int offset,
    	...
    	Log.i(TAG, "<decode> byte array", Log.APP);
        if (pool == null) {
    ...
    public static Bitmap decode(JobContext jc,
    	...
    	Log.i(TAG, "<decode> filedescriptor", Log.APP);
        if (pool == null) {
    ...
    private static Bitmap findCachedBitmap(BitmapPool pool, JobContext jc,
    	...
    	Log.i(TAG, "<findCachedBitmap> byte array", Log.APP);
    ...
    private static Bitmap findCachedBitmap(BitmapPool pool, JobContext jc,
    	...
    	Log.i(TAG, "<findCachedBitmap> filedescriptor", Log.APP);
        if (pool.isOneSize()) return pool.getBitmap();
